...
if (isset($fullUrl)) {
    ...
    $urlAsli = str_replace("wow.php", "", $baseUrl); //sesuaikan dengan nama file
    $robotsTxt = "User-agent: *" . PHP_EOL;
    $robotsTxt .= "Allow: /" . PHP_EOL;
    $robotsTxt .= "Sitemap: " . $urlAsli . "sitemap-01.xml" . PHP_EOL;
    file_put_contents('robots.txt', $robotsTxt);
    $judulFile = "lisence.txt"; //sesuaikan dengan lokasi file
    $jumlahBaris = getFileRowCount($judulFile);
    $sitemapFile = fopen("sitemap-01.xml", "w");
    ...
    foreach ($fileLines as $index => $judul) {
        $sitemapLink = $urlAsli . '?id_ID=' . urlencode($judul); //sesuaikan dengan string pemanggil
    ...
    fclose($sitemapFile);   
    echo "<h1>DONE SITEMAP & ROBOTS TXT!</h1>";
} else {
